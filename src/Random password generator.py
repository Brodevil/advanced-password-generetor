...
def store_password_in_file(user__name, info__pourpose, pass_word):
    password_stored_file = open(
        "E:\ADMIN\personal file\Abhinav\Passwords.txt", "a")
    file_write = str(present_time) + "\t:" + user__name + "\t:" + \
...
read_or_write = int(input(
    "Enter 1 to generate the password or Enter 2 to See the your password file history:\t"))
if read_or_write == 1:
    ...
    try:
        length = int(
            input("Just Enter a password length or just enter to use default length: \t"))
